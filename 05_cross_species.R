...
# 对外口径：人类按供体 S1–S4 标注（与正文 Table 1/2 及 Fig 4 一致；group 仅内部计算用）
human_summary <- seu_human@meta.data %>%
  ...
  mutate(species = paste0("Human ", sample)) %>%
  arrange(match(sample, c("S1", "S2", "S3", "S4"))) %>%
...
# 图 2：跨物种 DriftIndex 对比（小提琴）；人类按供体 S1–S4 标注
plot_df <- data.frame(
  species = c(as.character(seu_human$sample), rep("Mouse (healthy)", ncol(seu_mouse_bone))),
  DriftIndex = c(seu_human$DriftIndex, seu_mouse_bone$DriftIndex_mouse),
  stringsAsFactors = FALSE
)
plot_df$species <- factor(plot_df$species,
                          levels = c("S1","S2","S3","S4","Mouse (healthy)"))
...
ggplot(plot_df, aes(x = species, y = DriftIndex, fill = species)) +
  ...
  scale_fill_manual(values = c("S1"="#E41A1C","S2"="#FF7F00","S3"="#984EA3",
                               "S4"="#4DAF4A","Mouse (healthy)"="#377EB8")) +
...
bar_df$label <- factor(bar_df$label, levels = c("S1","S2","S3","S4","Mouse (healthy)"))
