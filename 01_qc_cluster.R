...
# 临床信息（2026-09-11 起对齐成稿口径：按样本 + 密度分层）
# S1 = GSM5201883: 61y 女，腰椎T=-3.0  → 骨质疏松范围（Osteoporotic range）
# S2 = GSM5201884: 45y 女，腰椎T=-1.3  → 骨量减少范围（Osteopenic range）
# S3 = GSM5201885: 66y 男，T值NA       → 未分层（无 BMD 测量），仅描述性 / 年龄对照
# S4 = GSM5201886: 31y 男，腰椎T=+0.6  → 未分层（ISCD 2023：T 值不用于 <50 岁男性诊断），仅描述性
# 只有 S1 vs S2 支持密度分层对比；S3/S4 不作跨组推断。
#
# 两组标签分工（勿混用）：
#   group_map   —— **内部分组**，仅供脚本内部排序/筛选，不对外出现；
#   stratum_map —— **对外口径**，用于结果表、图件与正文（Table 1/2、Fig S3、Fig S5）。
age_gender <- c(S1 = "61F", S2 = "45F", S3 = "66M", S4 = "31M")
t_lumbar   <- c(S1 = -3.0, S2 = -1.3, S3 = NA,  S4 = 0.6)
t_hip      <- c(S1 = -1.9, S2 = -1.2, S3 = NA,  S4 = -1.1)
group_map   <- c(S1 = "OP", S2 = "Osteopenia", S3 = "Age66M", S4 = "Normal")   # 内部用
stratum_map <- c(S1 = "Osteoporotic range", S2 = "Osteopenic range",
                 S3 = "Not stratified",     S4 = "Not stratified")             # 对外用
cat("\n[样本信息] 每样本 + 密度分层（S1/S2 可分层；S3/S4 未分层，仅描述性）:\n")
print(data.frame(AgeSex = age_gender, Lumbar_T = t_lumbar, Hip_T = t_hip,
                 InternalGroup = group_map, Stratum = stratum_map))
