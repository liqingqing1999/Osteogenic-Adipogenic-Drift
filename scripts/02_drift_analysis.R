...
# 对外口径：分组列改用密度分层标签（与正文 Table 1/2 一致；group 仅内部计算用）
STRATUM_OF <- c(S1 = "Osteoporotic range", S2 = "Osteopenic range",
                S3 = "Not stratified",     S4 = "Not stratified")
summary_out <- summary_by_sample %>%
  mutate(stratum = unname(STRATUM_OF[as.character(sample)])) %>%
  arrange(match(sample, c("S1", "S2", "S3", "S4"))) %>%
  select(sample, stratum, n_cells, DriftIndex_mean, DriftIndex_sd,
         MD_score_mean, SASP_score_mean, n_hybrid, hybrid_pct,
         n_osteo, osteo_pct, n_adipo, adipo_pct)
print(as.data.frame(summary_out))
write.csv(summary_out, file.path(tab_dir, "02_drift_summary.csv"), row.names = FALSE)
